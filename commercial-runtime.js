(function(){
  ...
  function previewRemainingFor(featureId){
    ...
    if(!preview)return 0;
    const entry=preview.byFeature?.[featureId];
    if(entry&&typeof entry.remaining==='number')return Number(entry.remaining);
    return Number(preview.remaining||0);
  }
  ...
  const freeAccess=()=>({
    ...
    freeLimits:{recentDrawsPerGame:Number(config.freeRecentDraws||20),savedCombinations:3,simulations:'unlimited',recurringProCredits:0},
  ...
  async function completeProAction(action){
    ...
    emit();notePreview(response.preview);
  ...
  async function runProAction(rawAction){
    ...
    if(typeof navigator!=='undefined'&&navigator.onLine===false){
      window.showFeedback?.(T('Нет подключения к интернету'),T('Для использования бесплатной PRO-пробы требуется подключение к интернету.'),'📡',4600);
      return null;
    }
    const access=await resolveProAccess(action);
    if(access===PRO_ACCESS.ACTIVE_PRO){
      try{return await completeProAction(action);}catch(error){return handleBackendError(error,action.feature);}
    }
    if(access===PRO_ACCESS.AUTH_REQUIRED){
      window.showFeedback?.(T('Требуется вход'),T('Войдите в аккаунт, чтобы бесплатные PRO-пробы сохранялись на всех устройствах.'),'👤',5200);
      return null;
    }
    ...
    if(access===PRO_ACCESS.COMMERCIAL_UNAVAILABLE||access===PRO_ACCESS.ERROR){
      window.showFeedback?.(T('Сервис временно недоступен'),T('PRO-операцию сейчас нельзя выполнить. Бесплатная проба не была использована.'),'⚠️',4800);
      return null;
    }
    const remaining=previewRemainingFor(action.previewFeature);
    if(access===PRO_ACCESS.PURCHASE_REQUIRED){track('pro_preview_exhausted',{feature:action.feature,previewFeature:action.previewFeature});return showPreviewOffer(action,0);}
    track('pro_preview_offer_shown',{feature:action.feature,remaining});
    return showPreviewOffer(action,remaining);
  }
  function previewStateLabel(remaining){
    return remaining>=3?'Осталось 3 бесплатные PRO-пробы'
      :remaining===2?'Осталось 2 бесплатные PRO-пробы'
      :'Осталась 1 бесплатная PRO-проба';
  }
  function showPreviewOffer(action,remaining){
    return new Promise(resolve=>{
      ...
      if(stateEl)stateEl.textContent=T(exhausted?'Бесплатные PRO-пробы использованы':previewStateLabel(remaining));
      if(useButton){useButton.hidden=exhausted;useButton.disabled=false;useButton.removeAttribute('aria-busy');useButton.textContent=T('Использовать бесплатную пробу');}
      ...
      if(useButton)useButton.onclick=async()=>{
        ...
        track('pro_preview_started',{feature:action.feature,remaining});
        try{const response=await completeProAction(action);finish(response);}
        catch(error){
          executing=false;useButton.disabled=false;useButton.removeAttribute('aria-busy');useButton.textContent=T('Повторить бесплатную пробу');
          if(error?.payload?.access){state.access=error.payload.access;emit();}
          if(error?.message==='preview_exhausted'){
            useButton.hidden=true;if(stateEl)stateEl.textContent=T('Бесплатные PRO-пробы использованы');
          }else{
            if(stateEl)stateEl.textContent=T('Не удалось выполнить PRO-операцию. Проба не использована.');
            handleBackendError(error,action.feature,{openPurchase:false});
          }
        }
      };
  ...
  function notePreview(preview){
    if(!preview||preview.charged!==true)return;
    const remaining=Number(preview.remaining||0);
    const message=remaining>=2?'Результат готов. Осталось 2 бесплатные PRO-пробы.'
      :remaining===1?'Результат готов. Осталась 1 бесплатная PRO-проба.'
      :'Результат готов. Все бесплатные PRO-пробы использованы.';
    track(remaining>0?'pro_preview_succeeded':'pro_preview_exhausted',{remaining});
  ...
  function openPaywall(feature=''){
    ...
    if(context){
      const labels={
        ...
        preview_sign_in:'3 бесплатные PRO-пробы. Создайте аккаунт или войдите по ссылке из e-mail, чтобы получить их — пробы сохранятся на всех ваших устройствах.',
        preview_exhausted:'Вы использовали все бесплатные ознакомительные PRO-пробы. Оформите PRO, чтобы продолжить.',
  ...
  async function backendJudge(inputRows,mountId='jc-mount',options={}){
    const context=backendContext();if(!context)return null;
    const rows=(inputRows||context.rows||[]).map(row=>({
  ...
  function handleBackendError(error,feature,{openPurchase=true}={}){
    if(error?.payload?.access)state.access=error.payload.access;
    ...
    if(message==='insufficient_history'||message==='history_unavailable'){
      window.showFeedback?.(T('Исторические данные недоступны'),T('Анализ невозможен: история тиражей не загружена. Обновите официальные результаты. Проба не списана.'),'⚠️',5200);
      return null;
    }
    if(message==='insufficient_rows'||message==='incomplete_result'){
      window.showFeedback?.(T('Не удалось сформировать ряды'),T('Сервис не смог собрать нужное число рядов. Попробуйте ещё раз — проба не списана.'),'⚠️',4600);
      return null;
    }
    window.showFeedback?.(T('Не удалось получить результат'),T('Попробуйте ещё раз — бесплатная проба не была использована.'),'⚠️',4200);
    return null;
  }
  ...
  window.LotoCommercial={
    ...
    policy:FEATURE_POLICY,freeGenerationModels:Object.freeze([...FREE_GENERATION_MODELS]),freeGenerationRowsPerRun:FREE_GENERATION_ROWS_PER_RUN,
