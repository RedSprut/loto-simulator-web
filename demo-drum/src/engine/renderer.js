...
export class RenderEngine {
  constructor(canvas, preset) {
    ...
    // WebGL context-loss resilience: if the GPU drops the context (memory pressure,
    // driver reset, tab/app backgrounding), calling preventDefault() lets the browser
    // RESTORE it — otherwise it's permanently dead and the whole page reloads. We pause
    // rendering while lost; three.js re-uploads its GL resources on restore. This keeps a
    // transient loss from bouncing the user back to the app's home screen.
    this._contextLost = false;
    canvas.addEventListener('webglcontextlost', (e) => { e.preventDefault(); this._contextLost = true; }, false);
    canvas.addEventListener('webglcontextrestored', () => { this._contextLost = false; }, false);
  ...
  render() {
    if (this._contextLost) return; // don't issue GL calls on a lost context; resume on restore
