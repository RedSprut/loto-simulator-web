...
export class CameraDirector {
  constructor(camera) {
    ...
    this._r3 = new THREE.Vector3();
    this._u3 = new THREE.Vector3();
    this._f3 = new THREE.Vector3();
  ...
  /** Guarantee the SUBJECT ball is fully on-screen with a safe margin during its
   *  reveal — adapting to the real canvas aspect + reserved bottom band. Pulls the
   *  camera straight back along the current view dir just enough (never closer), so an
   *  edge ball (first/last rack seat) is never cropped by the zoom-in. Zero motion when
   *  the ball already fits (narrow games / wide frames). */
  _fitBall(ballPos, aspect, band) {
    const cam = this._fitCam;
    const r = CONFIG.ball.radius * 1.18;                  // ball + a little safe halo
    const mX = 0.10, mTop = 0.11, mBot = 0.11 + (band || 0); // NDC safe margins (extra at the nav band)
    const dir = this._t2.copy(this.pos).sub(this.look).normalize();
    const L = this.look;
    let dist = this.pos.distanceTo(this.look);
    for (let it = 0; it < 28; it++) {
      cam.fov = this.camera.fov; cam.aspect = aspect; cam.near = this.camera.near; cam.far = this.camera.far;
      cam.position.copy(dir).multiplyScalar(dist).add(L);
      cam.up.set(0, 1, 0); cam.lookAt(L);
      cam.updateMatrixWorld(true); cam.updateProjectionMatrix();
      cam.matrixWorld.extractBasis(this._r3, this._u3, this._f3); // camera right / up / back
      let over = 0;
      for (let sx = -1; sx <= 1; sx += 2) for (let sy = -1; sy <= 1; sy += 2) {
        const p = this._p3.copy(ballPos).addScaledVector(this._r3, sx * r).addScaledVector(this._u3, sy * r);
        const n = this._ndc.copy(p).project(cam);
        over = Math.max(over, Math.abs(n.x) - (1 - mX), n.y - (1 - mTop), (-1 + mBot) - n.y);
      }
      if (over <= 0.003) break;
      dist *= 1 + Math.min(0.15, over * 0.6 + 0.02);
    }
    this.pos.copy(dir).multiplyScalar(dist).add(L);
  }
  ...
  update(dt, state, focus) {
    ...
    if (exiting) {
      ...
      // Once the ball has arrived and is being presented (DISPLAY), guarantee it stays
      // fully framed with a safe margin — fixes first/last balls being cropped by the
      // zoom-in on tall/narrow embedded canvases. (TRANSIT stays a free follow.)
      if (state === State.DISPLAY && focus && focus.lengthSq() > 1e-6) this._fitBall(focus, aspect, band);
    }
